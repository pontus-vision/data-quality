 ...
 * @see SecretManager
 */
public abstract class GenerateFormatPreservingPatterns extends AbstractGeneratePattern {
     ...
     */
    protected Cipher cipher = new com.idealista.fpe.algorithm.ff1.Cipher();
    ...
    /**
     * Transform the encrypted/decrypted array of {@code int}s into the corresponding {@code String} representation.
     */
    protected StringBuilder transform(int[] data) {
    ...
    /**
     * Transform the {@code String} element into an array of {@code int}s for FF1 encryption/decryption.
     */
    protected int[] transform(List<String> strs) {
    ...
    /**
     * Verify the validity of encrypted/decrypted data by comparing to the maxRank value.
     *
     * @param modifiedData result of format-preserving encryption.
     * @return whether the encrypted data is lower to the max rank.
     */
    protected boolean isValid(int[] modifiedData) {
        int i = 0;
        while (i < numeralMaxRank.length && numeralMaxRank[i] == modifiedData[i]) {
            i++;
        }
        ...
        if (i == numeralMaxRank.length) {
            ...
            return modifiedData[i] < numeralMaxRank[i];
        }
    }
