...
public class ReplaceNumericStringTest {
    ...
    public void testParseParameter() {
        ...
        assertTrue("Parameters should be valid", replaceNumericString.validParameters()); //$NON-NLS-1$
    }
    ...
    public void letterInParameter() {
        try {
            rns.parse("0X", false);
            fail("should get exception with input " + Arrays.toString(rns.getParsedParameters())); //$NON-NLS-1$
        } catch (Exception e) {
